...
const plannerShapeLabel = computed(() => `${displayColumns.value.length} × ${displayRows.value.length}`);
...
function submit() {
  ...
  form.transform((data) => ({
    ...
    cell_size_cm: Math.max(10, Math.min(200, Math.round(Number(data.cell_size_cm || 30)))),
    cells: form.cells,
  }));
...
<template>
  <section class="mb-8">
    <form class="space-y-6 rounded-[2rem] border border-border/70 bg-card/95 p-4 shadow-soft sm:p-6" @submit.prevent="submit">
      <section class="rounded-[1.9rem] border border-primary/15 bg-linear-to-br from-primary/12 via-background to-secondary/35 p-5 shadow-sm">
        <div class="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
          <div class="min-w-0">
            ...
            <p class="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">
              Lisa ruute otse kaardile, vali sobiv lahter ja ehita kuju samm-sammult. Tühjale lahtrile vajutades lisad uue ruudu, valitud ruudu ümber olevad nooled aitavad kuju kiiresti laiendada.
            </p>
          </div>

          <div class="grid gap-2 sm:grid-cols-3 lg:w-[19rem] lg:grid-cols-1">
      ...
      <section class="grid gap-4 xl:grid-cols-[minmax(0,1fr)_22rem]">
        <div class="space-y-4">
          <div class="grid gap-4 lg:grid-cols-[minmax(0,1fr)_17rem]">
            <div class="rounded-[1.75rem] border border-border/80 bg-background/80 p-4 shadow-sm">
              ...
              <label class="mt-4 mb-1.5 block text-sm font-medium text-foreground">Asukoht</label>
              <input
              ...
              <label class="mt-4 mb-1.5 block text-sm font-medium text-foreground">Ühe ruudu mõõt (cm)</label>
              <input
              ...
              <p class="mt-1 text-xs text-muted-foreground">
                See määrab, kui suur üks peenraruut päris aias on. Näiteks 30 cm teeb aiaplaani mõõtkava realistlikuks.
              </p>
              <p v-if="form.errors.cell_size_cm" class="mt-1 text-sm text-red-600">{{ form.errors.cell_size_cm }}</p>
            </div>

            <div class="rounded-[1.75rem] border border-border/80 bg-background/80 p-4 shadow-sm">
              <div class="flex items-center justify-between gap-3">
                <div>
                  <p class="text-sm font-semibold text-foreground">Peenra pilt</p>
                  <p class="mt-1 text-xs leading-5 text-muted-foreground">Soovi korral lisa väike visuaalne meeldetuletus.</p>
                </div>
                <span class="material-symbols-outlined rounded-2xl bg-primary/10 p-2 text-primary">photo_camera</span>
              </div>

              <input
                type="file"
                accept="image/*"
                class="mt-3 w-full rounded-2xl border border-border/80 bg-background px-4 py-3 text-sm text-foreground file:mr-3 file:rounded-xl file:border-0 file:bg-primary/10 file:px-3 file:py-2 file:font-medium file:text-primary hover:file:bg-primary/20"
              ...
              <div v-if="newBedImagePreview || existingImageUrl" class="mt-3 overflow-hidden rounded-[1.5rem] border border-border/80 shadow-sm">
                <div class="h-36 w-full bg-cover bg-center" :style="{ backgroundImage: `url('${newBedImagePreview ?? existingImageUrl}')` }" />
              </div>
              <p v-if="form.errors.image" class="mt-1 text-sm text-red-600">{{ form.errors.image }}</p>
            </div>
          </div>

          <section class="rounded-[1.9rem] border border-border/80 bg-linear-to-br from-background via-background to-primary/[0.03] p-4 shadow-sm sm:p-5">
            <div class="mb-4 flex flex-wrap items-start justify-between gap-3">
              <div class="min-w-0">
                ...
                <p class="mt-3 text-lg font-semibold tracking-tight text-foreground">Ehita kuju otse tööpinnal</p>
                <p class="mt-1 text-sm leading-6 text-muted-foreground">
                  Vajuta tühjale lahtrile, et ruut lisada. Vali olemasolev ruut, et seda edasi laiendada või eemaldada.
                </p>
              </div>

              <div class="flex items-center gap-2 rounded-2xl border border-border/80 bg-card/90 px-3 py-2 shadow-xs">
                <span class="material-symbols-outlined rounded-xl bg-primary/10 p-2 text-primary">view_in_ar</span>
                <div class="text-xs text-muted-foreground">
                  Kujundad praegu
                  <div class="text-base font-semibold text-foreground">{{ activeCells.length }} ruutu</div>
                </div>
              </div>
            </div>
            ...
            >
              <div class="mb-3 flex items-center justify-between gap-3 text-xs text-muted-foreground">
                <span>Valitud ruut: <span class="font-semibold text-foreground">{{ selectedCellLabel }}</span></span>
                <span>{{ selectedPlantCount }} taimekirjet</span>
              </div>
                      ...
                      <template v-if="getCellAt(x, y)">
                        <button
                          ...
                          :class="[
                            ...
                            highlightedCellId === getCellAt(x, y)?.id ? 'scale-[1.04] shadow-lg shadow-primary/20' : '',
                          ]"
                        ...
                        >
                          ...
                          >
                            {{ displayColumnNumber(x) }},{{ displayRowNumber(y) }}
                          </span>
                        </button>
            ...
            <div class="mt-4 grid gap-4 lg:hidden">
              <div class="rounded-[1.5rem] border border-border/80 bg-card/95 p-4 shadow-sm">
        ...
        <aside class="space-y-4">
          <div class="rounded-[1.75rem] border border-border/80 bg-card/95 p-4 shadow-sm">
          ...
          <div class="rounded-[1.75rem] border border-border/80 bg-card/95 p-4 shadow-sm">
            <div class="flex items-center justify-between gap-3">
              <p class="text-sm font-semibold text-foreground">Ruutu puudutav info</p>
              <span class="rounded-xl bg-primary/10 px-2.5 py-1 text-xs font-medium text-primary shadow-xs">{{ selectedPlantCount }}</span>
            </div>

            <div class="mt-4 grid gap-3">
              <div class="rounded-[1.25rem] border border-border/70 bg-background/70 p-3">
                <p class="text-[11px] font-semibold uppercase tracking-[0.16em] text-muted-foreground">Taimed ruudus</p>
              ...
              <div class="rounded-[1.25rem] border border-border/70 bg-background/70 p-3">
                <p class="text-[11px] font-semibold uppercase tracking-[0.16em] text-muted-foreground">Näpunäide</p>
                <p class="mt-1 text-sm text-muted-foreground">
                  Kui puudutad täidetud koha kõrval tühja lahtrit, saad peenra kuju samm-sammult edasi kasvatada.
                </p>
              </div>
            </div>
            ...
            <button
              type="button"
              class="mt-4 w-full rounded-2xl border px-3 py-2.5 text-sm font-medium"
              :class="selectedHasPlants || activeCells.length <= 1 ? 'border-border/60 bg-muted/30 text-muted-foreground cursor-not-allowed' : 'border-border bg-background text-foreground hover:bg-muted/50'"
              :disabled="selectedHasPlants || activeCells.length <= 1"
              @click="removeSelectedCell"
            >
      ...
      <div class="sticky bottom-3 z-10 -mx-1 rounded-[1.75rem] border border-border/70 bg-card/95 p-3 shadow-soft backdrop-blur sm:static sm:mx-0 sm:border-0 sm:bg-transparent sm:p-0 sm:shadow-none sm:backdrop-blur-0">
