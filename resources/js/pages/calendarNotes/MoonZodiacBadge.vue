...
const theme = computed(() => {
    switch (zodiac.value.biodynamicDayType) {
        case 'leaf':
            return {
                bg: 'var(--bio-leaf-bg)',
                accent: 'var(--bio-leaf-accent)',
                icon: 'eco',
            };
        case 'fruit':
            return {
                bg: 'var(--bio-fruit-bg)',
                accent: 'var(--bio-fruit-accent)',
                icon: 'nutrition',
            };
        case 'root':
            return {
                bg: 'var(--bio-root-bg)',
                accent: 'var(--bio-root-accent)',
                icon: 'grass',
            };
        default:
            return {
                bg: 'var(--bio-flower-bg)',
                accent: 'var(--bio-flower-accent)',
                icon: 'local_florist',
            };
    }
});
...
<template>
    <div
        class="mzSeg mzSeg--stacked mzSeg--dashboard h-full"
        :style="{ '--seg-bg': theme.bg, '--seg-accent': theme.accent }"
    >
        <div class="mzSeg-icon">
            <div
                class="flex size-8 shrink-0 items-center justify-center rounded-full bg-muted/40 ring-1 ring-border"
            >
                <span
