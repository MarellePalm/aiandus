...
<template>
    ...
    >
        <div class="h-24 w-24 shrink-0 overflow-hidden rounded-lg bg-gray-100">
            ...
            <div
                v-else
                class="flex h-full w-full items-center justify-center text-xs text-foreground/40"
            >
                Pole pilti
            </div>
        </div>
        ...
        <div class="flex min-w-0 flex-1 flex-col justify-between py-0.5">
            <div>
                <div class="flex items-start justify-between gap-3">
                    <h2
                        class="serif-italic min-w-0 flex-1 truncate text-sm leading-tight text-text-main sm:text-base md:text-xl"
                    >
                        {{ plant.name }}
                    </h2>
                    ...
                    <div class="relative" data-plant-menu @click.stop>
                        ...
                        <div
                            v-if="menuOpen"
                            class="absolute top-10 right-0 z-20 w-44 overflow-hidden rounded-2xl border border-border bg-card shadow-xl ring-1 ring-border"
                        >
                            <button
                                type="button"
            ...
            <div class="flex items-center gap-2">
                <span
                    ...
                    :class="statusStyles(plant.status).dot"
                ></span>
                <span
                    class="text-xs font-semibold tracking-wider uppercase"
                    :class="statusStyles(plant.status).text"
                >
                    {{ plant.status }}
                </span>
            </div>
